...
class RandomNumberTests(unittest.TestCase):
    ...
    def test_just_stop(self):
        ...
        class Thing:
            n = RandomNumber(5)
        thing = Thing()
        self.assertEqual(thing.n, 3)

    def test_each_access_is_different(self):
        random.seed(0)
        class Thing:
            n = RandomNumber(100)
        thing = Thing()
        self.assertEqual(thing.n, 49)
        self.assertEqual(thing.n, 97)
        self.assertEqual(thing.n, 53)
        self.assertEqual(thing.n, 5)

    def test_accesses_on_different_instances(self):
        random.seed(0)
        class Thing:
            n = RandomNumber(100)
        thing1 = Thing()
        thing2 = Thing()
        self.assertEqual(thing1.n, 49)
        self.assertEqual(thing2.n, 97)
        self.assertEqual(thing1.n, 53)
        self.assertEqual(thing2.n, 5)

    def test_two_uses_on_same_class(self):
        random.seed(0)
        class Thing:
            one = RandomNumber(0, 10)
            two = RandomNumber(20, 30)
        thing = Thing()
        self.assertNotEqual(thing.one, thing.two)
        self.assertGreaterEqual(thing.one, 0)
        self.assertLessEqual(thing.one, 10)
        self.assertGreaterEqual(thing.two, 20)
        self.assertLessEqual(thing.two, 30)
